#Getting features generated from GLCM for training images and creating trainingMatrix
fileList <-list.files(path="E:/Concordia projects/INSE 6180(Data Mining)/Code/output_glcm_train")

trainingMatrix <- matrix(data = 0 ,nrow = 4048, ncol = 9409 )
j=0
for(i in fileList)
{
  j=j+1
  fileName <- paste("E:/Concordia projects/INSE 6180(Data Mining)/Code/output_glcm_train/",i,sep ="")
  featureVector <- readRDS(file = fileName)
  trainingMatrix[j,] <- featureVector
}

#Getting features generated from GLCM for testing images and creating testingMatrix
fileList1 <-list.files(path="E:/Concordia projects/INSE 6180(Data Mining)/Code/output_glcm_test")
testingMatrix <- matrix(data = 0, nrow = 1640, ncol = 9409)
j=0
for(i in fileList1)
{
  j=j+1
  fileName1 <- paste("E:/Concordia projects/INSE 6180(Data Mining)/Code/output_glcm_test/",i,sep ="")
